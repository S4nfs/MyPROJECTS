...
const arr3 = [5, 6, 2]
const sum = arr3.reduce((accumulator, currElem, index, arr) => {
  //accumulator OR previousValue means Collecting
...
const newPriceTag = prices.filter((elem, index) => {
  //filter() = return an array of the given elements
  return elem < 400
})
...
const prices = [200, 300, 350, 400, 450, 500, 600]
const findElementoz = prices.find((currVal) => {
  //find() = problem is that it only returns one element, here 200
  return currVal < 400
})
...
function getIndexToIns(arr) {
  function compareNumbers(a, b) {
    // -ve return value means 'a' first and +ve return value means 'b' first
...
//✔️ flat() - create a new array with the sub-array elements concatenated
const myflatArr = [1, 2, [3, [4, 5, 6], 7], 8]
const newflatArr = myflatArr.flat(2) //how deep a nested array should be flattened default is 1
console.log(newflatArr)

//✔️ flatMap() - maps all array elements and creates a new flat array
const myflatMapArr = [1, 2, 3, 4, 5, 6]
const newflatMapArr = myflatMapArr.flatMap((x) => x * 2)
...
let res = personnel
  .filter((jedi) => jedi.isForceUser)
  .map((jedi) => jedi.pilotingScore + jedi.shootingScore)
  .reduce((acc, currElement) => acc + currElement)
console.log(res)
