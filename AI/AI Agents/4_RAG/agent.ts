...
export async function callAgent(client: MongoClient, query: string, thread_id: string) {
  ...
  // const stream: any = await app.streamEvents(
  //   {
  //     messages: [new HumanMessage(query)],
  //   },
  //   { version: 'v2' as const, recursionLimit: 15, configurable: { thread_id: thread_id } }
  // )
  // for await (const event of stream) {
  //   console.log('EVENTS:', event.event, '|||||||', '\n', JSON.stringify(event.data.chunk))
  //   if (event.event === 'on_chat_model_stream') {
  //     console.log('Streaming Data', JSON.stringify({ type: 'response', data: event.data.chunk.agent?.messages?.[0] }))
  //   }
